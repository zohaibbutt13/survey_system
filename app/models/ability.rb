class Ability
  ...
  def initialize(user)
    ...
    if user.admin?
      ...
      can :calculate_surveys, User, company_id: user.company_id
      can [:read, :create, :update, :destroy, :edit, :new, :add_question, :add_option, :delete_question, :delete_option, :survey_charts], Survey, company_id: user.company_id, user_id: user.id if user.company.company_setting.supervisors_survey_permission?
      can [:read, :create], UserResponse
    elsif user.member?
      can [:update, :edit], user
